...
with st.sidebar:
    ...
    st.divider()
    # ダウンロードボタンをサイドバーに固定し、どのタブにいても常に現在の全設定を保存
    st.download_button(
        "📥 現在の全設定を保存する", 
...
def get_persisted_df(key, d_df, categories=None):
    ...
    if key in tables:
        ...
        # （これによりスタッフ名の順番や名前自体が変更されても、行に設定されていたデータは確実に新スタッフに引き継がれます）
...
with tab_roster:
    ...
    # 【レイアウト改善】左右並びを廃止し、上下並びに変更。横幅全体を使って横スクロールを不要にします。
